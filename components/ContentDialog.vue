<template>
  ...
  >
      ...
      <v-card-title class="pa-0">
        <v-toolbar dark color="primary">
          ...
          <v-btn
            icon
            dark
            :disabled="navigationButtons.firstItem"
            @click="firstItem()"
          >
            <v-icon :size="$vuetify.breakpoint.mdAndUp ? 35 : 20">
              mdi-skip-previous
            </v-icon>
          </v-btn>
          <v-btn
            icon
            dark
            :disabled="navigationButtons.backward"
            @click="backward()"
          >
            <v-icon :size="$vuetify.breakpoint.mdAndUp ? 60 : 40">
              mdi-menu-left
            </v-icon>
          </v-btn>
          ...
          <div v-if="index === -99" :class="$vuetify.breakpoint.mdAndUp ? 'text-h6' : 'text-body-2'">
            1 de 1
          </div>

          <div v-if="index !== -99" :class="$vuetify.breakpoint.mdAndUp ? 'text-h6' : 'text-body-2'">
            {{ cIndex }} de {{ cAmount }}
            <!-- {{ index + 1 }} de {{ lastPagePagination }} -->
          </div>

          <v-btn
            icon
            dark
            :disabled="navigationButtons.forward"
            @click="forward()"
          >
            <v-icon :size="$vuetify.breakpoint.mdAndUp ? 60 : 40">
              mdi-menu-right
          ...
          <v-btn
            icon
            dark
            :disabled="navigationButtons.lastItem"
            @click="lastItem()"
          >
            <v-icon :size="$vuetify.breakpoint.mdAndUp ? 35 : 20">
              mdi-skip-next
      ...
      <v-card-text id="toPrint">
        <v-container grid-list-xl>
          <Content-Note-Card
            ...
            :id="idMegamedia !== undefined ? idMegamedia : null"
            ...
            :fontColor="colorText"
            :icon="icon"
            :idMegamedia="cIdNoticia !== undefined ? cIdNoticia : id"
          ...
          <Content-Image-Card
            ...
            "
            :image="imageObj.newSRC !== undefined ? imageObj.newSRC : imageObj.imageSrc"
            ...
            :_id="imageObj._id"
            :idMegamedia="
              imageObj.isNewId != undefined ? imageObj.isNewId : imageObj._id
            "
          ...
          <Content-PDF-Card
            ...
            :material="null"
            :idMegamedia="
              pdfObject.isNewId != undefined ? pdfObject.isNewId : pdfObject._id
            "
            :published="published != undefined ? published : null"
            :selleable="selleable != undefined ? selleable : null"
            :relatedPDFs="relatedPDFs"
      ...
      <v-card flat>
        <v-container grid-list-xl>
          ...
          <ContentImageCardPrint
            ...
            :location="imageObj.place != undefined ? imageObj.place : null"
            :photographer="
              imageObj.photographer != undefined ? imageObj.photographer : null
            "
            :clasification="
              imageObj.clasificationRef != undefined
                ? imageObj.clasificationRef.name
    ...
    <ContentPDFDiaog
      ref="contentPDFDiaog"
      :pdfSrc="pdfObject.newSRCPDF !== undefined ? pdfObject.newSRCPDF : pdfObject.imageSrc"
...
export default {
    ...
    forwardElement() {
      ...
      if (this.searchedItems && this.searchedItems.length > 0) {
        ...
        try{
          ...
          fowardIndex = ((fowardIndex)>this.searchedItems.length-1) ? 0:fowardIndex
        ...
        if (temp) {
          if (
            ...
            console.log("////////// -------CREATING FORWARD END------ //////////")
  ...
  methods: {
    ...
    async openDialog(note, mode, index, searchType, formatedYear) {
      ...
      const cItems = this.getItemsOfArray();
      if(cItems.length < this.lastPagePagination){
        this.lastPagePagination = cItems.length;
      }
      ...
      // this.searchLastItem();
      if (
        this.index === cItems.length - 1 &&
        this.$parent.page === this.$parent.pages[this.$parent.pages.length - 1]
      ) {
    ...
    getNewCustomIndex(){
      ...
      this.cAmount = this.$parent.elementsCounter;
      const cIndex = ((page - 1) * this.$parent.totalResult) + this.index + 1;
    ...
    async getNote(id, year) {
      ...
      this.cIdNoticia = response.data.note.idMegamedia ?? response.data.note.isNewId;
      console.dir(response.data)
      this.idMegamedia =
        response.data.note.isNewId || response.data.note._id;
      ...
      if (response.data.note.content != undefined) {
        ...
        if (this.$route.path == "/advSearch") {
          ...
          words.forEach((element) => {
            console.log(`advSearch.keywords - ${advSearch.keywords}`)
            ...
            this.title = this.addStrongText(this.title, element)
      ...
      if (response?.data?.note?.noteBookRef) {
   ...
   //   this.content = this.content.replaceAll("<br><br>", "<br>");
    ...
    async getImage(id, year) {
      ...
      // this.agencyimg = this.imageObj.idAgency != undefined && this.imageObj.idAgency != null ? this.imageObj.idAgency : '';
      this.agencyimg = this.imageObj !== null ? this.imageObj.idAgency?.name : null;
      this.material = this.imageObj.material;
      this.emailtype = 'images'
      this.fileEmailSrc= this.imageObj.imageSrc

      if( this.imageObj?.photographer){
        this.photographer = this.imageObj.photographer?.name + ' ' + this.imageObj.photographer?.lname;
      } else {
    ...
    openWorkbench() {
      switch (this.mode) {
        case 1:
          ...
          this.$refs.addWorkbenchDialog.openDialog("note", this.id, this.publishDate);
          break;

        case 2:
          this.$refs.addWorkbenchDialog.openDialog("image", this.imageObj._id, this.imageObj.date);
          break;

        case 3:
          this.$refs.addWorkbenchDialog.openDialog("pdfs", this.pdfObject._id, this.pdfObject.dateCreation);
          break;
      }
    },
    // setFontSize(size) {
    ...
    getItemsOfArray() {
      switch(this.mode){
        case 1:
          return this.$parent.searched;
        case 2:
          return this.$parent.images;
        case 3:
          return this.$parent.pdfs;
      }
    },
    getCustomYear(mode, cItem){
      switch(Number(mode)) {
        case 1:
          return new Date(cItem.date).getFullYear();
        case 2:
          return new Date(cItem.publicationDate).getFullYear();
        case 3:
          return new Date(cItem.datePublication).getFullYear();
      }
    },
    // NAVIGATION METHODS //////////////
    async forward() {
      ...
      let items = this.getItemsOfArray();
      this.index = this.index + 1;
      // console.log(`this is< index + 1 from forward ${this.index}`)
      let page = this.$parent.page;
      let lastPage = this.$parent.pages[this.$parent.pages?.length - 1];


      if(this.index >= items.length && page <= lastPage){
        this.index = 0;
        this.$parent.page = this.$parent.page + 1;
        page = this.$parent.page;
        this.$parent.advObjNotMove.quantity = page;
        await this.$parent.getAdvSearch();
        items = this.getItemsOfArray();
      }
      this.lastPagePagination = items.length;
      try {
        ...
        const _id = items[this.index]._id;
        const cItem = items[this.index];
        const cYear = this.getCustomYear(this.mode, cItem);
        await this.getInformation(this.mode, _id, this.index, cYear);

        this.getNewCustomIndex();

        if (
          this.index + 1 === items.length &&
          page === lastPage
        ) {
          this.disabledLastItemAndForward();
        }
    ...
    async lastItem() {
      ...
      try {
        ...
        //     // this.refreshFatherPage(Number(this.mode));
        // } else {
        //     this.$parent.page = lastPage;
        //     this.$parent.advObjNotMove.quantity = lastPage;
        //     await this.$parent.getAdvSearch(true);
        //     items = this.getItemsOfArray();
        // }
        ...
        const _id = items[items.length - 1]._id;
        const cItem = items[items.length - 1];
        const cYear = this.getCustomYear(this.mode, cItem);
        await this.getInformation(this.mode, _id, this.index, cYear);
        this.getNewCustomIndex();
        this.disabledLastItemAndForward();
    ...
    async firstItem() {
      ...
      try {
        ...
        this.lastPagePagination = items.length + 1;
        const _id = items[0]._id;
        const cItem = items[0];
        const cYear = this.getCustomYear(this.mode, cItem);
        await this.getInformation(this.mode, _id, this.index, cYear);
        this.getNewCustomIndex();
    ...
    addStrongText(text, simpleText) {
      //se cambio de luegar el algoritmo
      return text;
