...
<script>
  ...
  export default {
    ...
    computed: mapState(["logged", "snackText", "snackBar", "username", 'img', 'img2']),
    methods : {
      openDialog(){
        this.getUserInfo();
        this.dialog = true;
      ...
      async getUserInfo(){
        try {
          ...
          sessionStorage.setItem("imgC", this.blobFile);
      ...
      async editUserProfile(){
        try {
          ...
          const response = await this.$axios.$put('/users/' + localStorage.user_idC, body,{
            headers : { Authorization : 'Bearer ' + localStorage.tokenC },
